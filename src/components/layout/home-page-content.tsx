...
export default function HomePageContent() {
  ...
  }, [isAuthenticated, allVideos, userRole, isOnline]); // Re-fetch if auth state, videos, role or online status changes
  ...
  const fetchRecommendations = async () => {
    ...
    try {
      const input: RecommendVideosInput = {
        userWatchHistory: mockUserWatchHistory, // Use actual history in a real app
      ...
      setRecommendedVideosList([]); // Fallback to empty list on error
    } finally {
        ...
        <>
          {isRecommendationsLoading ? (
             <div className="mb-8">
              ...
              <VideoGridSkeleton /> {/* Show skeleton while recommendations load */}
            </div>
